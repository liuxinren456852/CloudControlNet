...
namespace utility
{
	...
	public:
		...
		// Estimate the normals of the input Point Cloud by PCL speeding up with OpenMP
		bool CalculateNormalVectorOpenMP(typename pcl::PointCloud<PointT>::Ptr inputPointCloud,
			float radius,
			pcl::PointCloud<pcl::Normal>::Ptr &normals);

		// Principle Component Analysis(PCA) of the Point Cloud with fixed search radius
		bool CalculatePcaFeaturesOfPointCloud(typename pcl::PointCloud<PointT>::Ptr inputPointCloud,
			float radius,
			std::vector<pcaFeature> &features);

		// Use OpenCV to accomplish the Principle Component Analysis(PCA)
	...
	private:
		...
		// Check the Normals (if they are finite)
	...
	*/
	template<typename PointT>
	bool PrincipleComponentAnalysis<PointT>::CalculatePcaFeature(typename pcl::PointCloud<PointT>::Ptr inputPointCloud,
		std::vector<int> &search_indices,
		pcaFeature &feature)
	{
		size_t ptNum;
		...
		{
			feature.curvature = feature.values.lamada3 / (feature.values.lamada1 + feature.values.lamada2 + feature.values.lamada3);   //曲率的算法是最小的 eigen value 除以三个 eigen value 之和;
	...
	*/
	template<typename PointT>
	void PrincipleComponentAnalysis<PointT>::CheckNormals(pcl::PointCloud<pcl::Normal>::Ptr &normals)
	{
