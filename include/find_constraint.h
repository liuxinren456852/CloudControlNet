...
namespace ccn
{
	...
	public:
		// Constraint Type 1
		void find_strip_adjacent_constraint(vector<vector<cloudblock_t, Eigen::aligned_allocator<cloudblock_t>>> &blocks_all, vector<constraint_t> &innerstrip_cons_all);
		void find_adjacent_constraint_in_strip(vector<cloudblock_t, Eigen::aligned_allocator<cloudblock_t>> &blocks_strip, vector<constraint_t> &innerstrip_cons);

		// Constraint Type 2
		void find_overlap_registration_constraint(vector<cloudblock_t, Eigen::aligned_allocator<cloudblock_t>> &blocks, vector<constraint_t> &registration_cons_all, int k_search_neighbo, double min_iou);
